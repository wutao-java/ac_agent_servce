"""HITL 恢复层。上下文压缩不能裁掉高风险 workflow checkpoint。"""
...
def create_workflow_checkpoint(request: ChatRequest, order: dict[str, Any], *, boundary: str) -> dict[str, Any]:
    workflow_id = f"wf-lesson35-{request.session_id}-{order_no(order)}"
...
def _blocked(request: ChatResumeRequest, answer: str, business_recheck: dict[str, Any], workflow: dict[str, Any] | None = None, agent_version: str = "lesson-35-context-compression") -> ChatResumeResponse:
    return ChatResumeResponse(session_id=request.session_id, workflow_id=request.workflow_id, status="blocked", answer=answer, workflow=workflow, result={"decision": request.decision, "accepted": False}, business_recheck=business_recheck, session_state={"agent_version": agent_version, "resume_boundary": "上下文压缩不能丢掉高风险 workflow checkpoint。"})
