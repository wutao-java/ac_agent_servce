...
from tools.tool_runtime import build_logistics_call, load_owned_order, make_tool_call
...
class AfterSaleWorkflow:
    ...
    def run(self, request: ChatRequest) -> AfterSaleWorkflowState:
        ...
        workflow_id = "wf-lesson32-{0}-{1}".format(request.session_id, order_id or "missing")
    ...
    def _load_order(self, state: AfterSaleWorkflowState) -> dict[str, Any]:
        ...
        order, order_call = load_owned_order(
            str(state["order_id"]),
    ...
    def _load_logistics(self, state: AfterSaleWorkflowState) -> dict[str, Any]:
        ...
        return self._complete(state, "load_logistics", {"tool_calls": [*state["tool_calls"], build_logistics_call(order, state["runtime_user_id"])]})
