...
class TraceStore:
    ...
    def add(self, session_id: str, event_type: str, payload: dict[str, Any]) -> TraceEvent:
        ...
        events.append(event)
        log_course_event("TRACE_EVENT", event.summary, teaching=event_type in {"runtime_context_built", "context_built", "tool_finished", "rag_pre_retrieved", "workflow_completed", "human_approval_required", "cost_recorded"}, trace_event=event_type, step=event.step, category=event.category, status=event.status, payload=event.payload)
        return event
